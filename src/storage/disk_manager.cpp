...
void DiskManager::write_page(int fd, page_id_t page_no, const char* offset, int num_bytes)
{
	...
	// finished but resLenght != num_bytes will fail the unit_test
	if (lseek(fd, page_no * PAGE_SIZE, SEEK_SET) == -1)
		throw InternalError("DiskManager::write_page Error");
	if (const int resLenght = write(fd, offset, num_bytes); resLenght == -1)
		throw InternalError("DiskManager::write_page Error");
}
...
void DiskManager::read_page(int fd, page_id_t page_no, char* offset, int num_bytes)
{
	...
	if (const int resLenght = read(fd, offset, num_bytes); resLenght == -1)
		throw InternalError("DiskManager::read_page Error");
}
...
int DiskManager::open_file(const std::string& path)
{
	...
	if (!this->is_file(path))
		throw FileNotFoundError(path);
	else if (!this->path2fd_.count(path))
	{
