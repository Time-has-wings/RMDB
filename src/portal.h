...
public:
    ...
    std::shared_ptr<PortalStmt> start(std::shared_ptr<Plan> plan, Context *context)
    {
        ...
        else if (auto x = std::dynamic_pointer_cast<DMLPlan>(plan))
        {
            ...
            case T_select:
            {
                ...
                std::unique_ptr<AbstractExecutor> root = convert_plan_executor(p, context, SQLType::select_sql);
                return std::make_shared<PortalStmt>(PORTAL_ONE_SELECT, std::move(p->sel_cols_), std::move(root), plan);
            }

            case T_Update:
            {
                std::unique_ptr<AbstractExecutor> scan = convert_plan_executor(x->subplan_, context, SQLType::update_sql);
                std::vector<Rid> rids;
            ...
            case T_Delete:
            {
                std::unique_ptr<AbstractExecutor> scan = convert_plan_executor(x->subplan_, context, SQLType::delete_sql);
                std::vector<Rid> rids;
                for (scan->beginTuple(); !scan->is_end(); scan->nextTuple())
                {
                    rids.emplace_back(scan->rid());
	...
	std::unique_ptr<AbstractExecutor> convert_plan_executor(const std::shared_ptr<Plan>& plan, Context* context, SQLType sql_type_)
    {
        if (auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan))
        {
            return std::make_unique<ProjectionExecutor>(convert_plan_executor(x->subplan_, context, sql_type_),
                                                        x->sel_cols_);
        }
        else if (auto x = std::dynamic_pointer_cast<ScanPlan>(plan))
        {
            if (x->tag == T_SeqScan)
            {
                return std::make_unique<SeqScanExecutor>(sm_manager_, x->tab_name_, x->conds_, context, sql_type_);
            }
            else
            {
                return std::make_unique<IndexScanExecutor>(sm_manager_, x->tab_name_, x->conds_, x->index_col_names_, context, sql_type_);
            }
        }
        else if (auto x = std::dynamic_pointer_cast<JoinPlan>(plan))
        {
            std::unique_ptr<AbstractExecutor> left = convert_plan_executor(x->left_, context, sql_type_);
            std::unique_ptr<AbstractExecutor> right = convert_plan_executor(x->right_, context, sql_type_);
        ...
        {
            return std::make_unique<SortExecutor>(convert_plan_executor(x->subplan_, context, sql_type_),
                                                  x->orders);
        }
        else if (auto x = std::dynamic_pointer_cast<LimitPlan>(plan))
        {
            return std::make_unique<LimitExecutor>(convert_plan_executor(x->subplan_, context, sql_type_),
                                                  x->orders, x->limit);
        }
        else if (auto x = std::dynamic_pointer_cast<GroupPlan>(plan))
        {
            return std::make_unique<GroupExecutor>(convert_plan_executor(x->subplan_, context, sql_type_), x->group_);
        }
