...
"");
EXPECT_EQ(fmt::to_string(bigint(0x42)),
"42");
EXPECT_EQ(fmt::to_string(bigint(0x123456789abcedf0)),
"123456789abcedf0");
}
...
n2.
square();
EXPECT_EQ(fmt::to_string(n2),
"ffffffffe000000001");
bigint n3(max_value<uint64_t>());
n3.
square();
EXPECT_EQ(fmt::to_string(n3),
"fffffffffffffffe0000000000000001");
bigint n4;
n4.assign_pow10(10);
EXPECT_EQ(fmt::to_string(n4),
"2540be400");
}
...
EXPECT_EQ(exact_str
.substr(0, 15), cache_str.substr(0, 15));
int diff = cache_str[15] - exact_str[15];
if (diff == 1)
EXPECT_GT(exact_str[16],
'8');
else
EXPECT_EQ(diff,
0);
...
EXPECT_EQ(exact_str
.substr(0, 16), cache_str.substr(0, 16));
}
}
}
...
handler()
.on_digit('0', 100, 99, 0, false);
EXPECT_THROW (handler()
.on_digit('0', 100, 100, 0, false), assertion_failure);
namespace digits = fmt::detail::digits;
EXPECT_EQ(handler(1)
.on_digit('0', 100, 10, 10, false), digits::error);
// Check that divisor - error doesn't overflow.
EXPECT_EQ(handler(1)
.on_digit('0', 100, 10, 101, false), digits::error);
// Check that 2 * error doesn't overflow.
uint64_t max = max_value<uint64_t>();
EXPECT_EQ(handler(1)
.on_digit('0', max, 10, max - 1, false), digits::error);
}
...
i<num_bits - 1; i++) {
n <<= 1;
EXPECT_EQ(fmt::detail::countl_zero(n - 1), num_bits
- i);
EXPECT_EQ(fmt::detail::countl_zero(n), num_bits
- i - 1);
}
}
...
struct double_double
{
	...
	auto operator-() const -> double_double
	{
		return double_double(-a, -b);
	}
};

auto format_as(double_double d) -> double
{
	return d;
}
...
struct slow_float
{
	...
	auto operator-() const -> slow_float
	{
		return slow_float(-value);
	}
};

auto format_as(slow_float f) -> float
{
	return f;
}
...
{
	template<>
	struct is_floating_point<double_double> : std::true_type
	{
	};
	template<>
	struct numeric_limits<double_double>
	{
	...
	template<>
	struct is_floating_point<slow_float> : std::true_type
	{
	};
	template<>
	struct numeric_limits<slow_float> : numeric_limits<float>
	{
	};
}  // namespace std
...
FMT_BEGIN_NAMESPACE
		...
		{
			template<>
			struct is_fast_float<slow_float> : std::false_type
			{
			};
			namespace dragonbox
			{
				template<>
				struct float_info<slow_float>
				{
