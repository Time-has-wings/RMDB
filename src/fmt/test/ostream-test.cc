...
auto format_as(unstreamable_enum e) -> int
{
	return e;
}
...
{
	template<>
	struct formatter<test_string> : ostream_formatter
	{
	};
	template<>
	struct formatter<date> : ostream_formatter
	{
	};
	template<>
	struct formatter<streamable_enum> : ostream_formatter
	{
	};
	template<>
	struct formatter<empty_test> : ostream_formatter
	{
	};
}  // namespace fmt
...
{
	MOCK_METHOD(std::streamsize, xsputn,
	(const void*, std::streamsize));
...
namespace fmt_test
{
	...
	auto operator<<(Output& out, abc) -> Output&
	{
		return out << "abc";
	}
}  // namespace fmt_test
...
{
	template<typename T>
	struct formatter<test_template < T>> : formatter<int>
{
...
template<>
struct formatter<fmt_test::abc> : ostream_formatter
{
};
}  // namespace fmt
...
{
	template<>
	struct formatter<abstract> : ostream_formatter
	{
	};
}  // namespace fmt
