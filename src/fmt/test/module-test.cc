...
L"{}", 42);
EXPECT_EQ(L"42",
std::wstring_view(wbuffer)
);

fmt::wmemory_buffer wb;
fmt::format_to(std::back_inserter(wb),
L"{}", 42);
EXPECT_EQ(L"42",
std::wstring_view(wbuffer)
);
...
wchar_t wbuffer[4] = { 0 };
fmt::vformat_to_n(wbuffer,
3, fmt::wstring_view(L"{:}"), wstore);
}
...
"{:L}", store));
std::string s;
fmt::vformat_to(std::back_inserter(s), classic,
"{:L}", store);
EXPECT_EQ("4.2", s);
...
L"{:L}", wstore));
std::wstring w;
fmt::vformat_to(std::back_inserter(w), classic,
L"{:L}", wstore);
EXPECT_EQ(L"4.2", w);
...
EXPECT_EQ("fmt", nsv);
EXPECT_TRUE(fmt::string_view("fmt")
== nsv);

fmt::wstring_view wsv(L"fmt");
EXPECT_EQ(L"fmt", wsv);
EXPECT_TRUE(fmt::wstring_view(L"fmt")
== wsv);
}
...
EXPECT_EQ("0x2a",
fmt::to_string(fmt::ptr(p)
));
std::unique_ptr<int> up(p);
EXPECT_EQ("0x2a",
fmt::to_string(fmt::ptr(up)
));
up.
release();
...
fmt::format_system_error(buffer,
0, "oops");
...
fmt::format("{0}", std::error_code(42,
...
fmt::format(L"{0}", std::error_code(42,
std::generic_category()
)));
}
...
EXPECT_EQ("1, 2, 3",
to_string(fmt::join(arr + 0, arr + 3, sep)
));
EXPECT_EQ("1, 2, 3",
to_string(fmt::join(arr, sep)
));
EXPECT_EQ("1, 2, 3",
to_string(fmt::join(vec.begin(), vec.end(), sep)
));
EXPECT_EQ("1, 2, 3",
to_string(fmt::join(vec, sep)
));
EXPECT_EQ("1, 2, 3",
to_string(fmt::join(il, sep)
));

auto wsep = fmt::wstring_view(L", ");
EXPECT_EQ(L"1, 2, 3", fmt::format(L"{}",
fmt::join(arr
+ 0, arr + 3, wsep)));
EXPECT_EQ(L"1, 2, 3", fmt::format(L"{}",
fmt::join(arr, wsep
...
EXPECT_TRUE(fmt::localtime(time_now)
.tm_year > 120);
EXPECT_TRUE(fmt::gmtime(time_now)
.tm_year > 120);
auto chrono_now = std::chrono::system_clock::now();
EXPECT_TRUE(fmt::gmtime(chrono_now)
.tm_year > 120);
}
...
EXPECT_EQ("42s", fmt::format("{}", std::chrono::seconds{
42}));
EXPECT_EQ("4.2µs", fmt::format("{:3.1}", us{
4.234}));
EXPECT_EQ("4.2µs",
fmt::format(std::locale::classic(),
"{:L}", us{
4.2}));

EXPECT_EQ(L"42s", fmt::format(L"{}", std::chrono::seconds{
42}));
EXPECT_EQ(L"4.2µs", fmt::format(L"{:3.1}", us{
4.234}));
EXPECT_EQ(L"4.2µs",
fmt::format(std::locale::classic(),
L"{:L}", us{
...
fmt::format(fg(fmt::terminal_color::black),
"{}", 42));
...
auto s = "fmt";
EXPECT_EQ(s, fmt::cstring_view(s)
.
c_str()
);
auto w = L"fmt";
EXPECT_EQ(w, fmt::wcstring_view(w)
.
c_str()
);
}
