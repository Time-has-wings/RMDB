...
{
	friend fmt::string_view to_string_view(to_stringable)
	{
		return {};
	}
};
...
store.
push_back(fmt::arg("band", std::cref(band))
);
band[9] = 'c';  // Changing band affects the output.
EXPECT_EQ(fmt::vformat("{band}", store),
"Rolling Scones");
}
...
struct copy_throwable
{
	...
	copy_throwable(const copy_throwable&)
	{
		throw "deal with it";
	}
};
...
store->
push_back(fmt::arg("a1", "foo")
);
auto moved_store = std::move(*store);
store.
reset();
EXPECT_EQ(fmt::vformat("{} {} {a1}", moved_store),
"42 foo foo");
}
