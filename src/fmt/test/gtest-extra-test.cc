...
redir.reset(nullptr);
write_copy.
dup2(write_fd);  // "undo" close or dtor will fail
}
...
copy.
dup2(fd);  // "undo" close or dtor will fail
}
...
write_copy.
dup2(write_fd);  // "undo" close or dtor of buffered_file will fail
}
