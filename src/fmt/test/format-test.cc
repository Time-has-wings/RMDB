...
<uint64_t>(n),
42);
EXPECT_EQ(uint128_fallback(1)
<< 112, uint128_fallback(0x1000000000000, 0));
EXPECT_EQ(uint128_fallback(0x1000000000000, 0)
>> 112, uint128_fallback(1));
}
...
EXPECT_EQ(n, uint128_fallback(1)
<< 64);
}
...
EXPECT_EQ(fmt::detail::parse_nonnegative_int(begin, end, -1),
-1);
s = "2147483649";
begin = s.begin();
end = s.end();
...
EXPECT_EQ(to_string(message), std::system_error(ec, "test")
.
what()
);
...
auto ec = std::error_code(EDOM, std::generic_category());
EXPECT_STREQ(test_error
.
what(), std::system_error(ec, "test")
.
what()
);
EXPECT_EQ(test_error
.
code(), ec
);
...
EXPECT_EQ(error
.
code(), std::error_code(EDOM, std::generic_category())
);
}
...
size(),
0);
EXPECT_EQ(std::string(&buffer2[0], buffer2.size()),
"testa");
EXPECT_GT(buffer2
.
capacity(),
...
EXPECT_EQ("test", fmt::format("test")); }
...
"**🤡**");
EXPECT_EQ(fmt::format("{:*^8}", "你好"),
"**你好**");
EXPECT_EQ(fmt::format("{:#6}", 42.0),
"   42.");
EXPECT_EQ(fmt::format("{:6c}", static_cast<int>('x')),
"x     ");
EXPECT_EQ(fmt::format("{:>06.0f}", 0.00884311),
"     0");
}
...
EXPECT_EQ(fmt::format("{:{}}", 42, short(4)),
"  42");
}
...
EXPECT_EQ(fmt::format("{:}", 0.0),
"0");
EXPECT_EQ(fmt::format("{:f}", 0.0),
"0.000000");
EXPECT_EQ(fmt::format("{:g}", 0.0),
"0");
EXPECT_EQ(fmt::format("{:}", 392.65),
"392.65");
EXPECT_EQ(fmt::format("{:g}", 392.65),
"392.65");
EXPECT_EQ(fmt::format("{:G}", 392.65),
"392.65");
EXPECT_EQ(fmt::format("{:g}", 4.9014e6),
"4.9014e+06");
EXPECT_EQ(fmt::format("{:f}", 392.65),
"392.650000");
EXPECT_EQ(fmt::format("{:F}", 392.65),
"392.650000");
EXPECT_EQ(fmt::format("{:L}", 42.0),
"42");
EXPECT_EQ(fmt::format("{:24a}", 4.2f),
"           0x1.0cccccp+2");
EXPECT_EQ(fmt::format("{:24a}", 4.2),
"    0x1.0cccccccccccdp+2");
EXPECT_EQ(fmt::format("{:<24a}", 4.2),
"0x1.0cccccccccccdp+2    ");
EXPECT_EQ(fmt::format("{0:e}", 392.65),
"3.926500e+02");
EXPECT_EQ(fmt::format("{0:E}", 392.65),
"3.926500E+02");
EXPECT_EQ(fmt::format("{0:+010.4g}", 392.65),
"+0000392.6");
char buffer[buffer_size];
...
if (
fmt::detail::const_check(std::numeric_limits<double>::is_iec559)
) {
double d = (std::numeric_limits<double>::min)();
EXPECT_EQ(fmt::format("{:a}", d),
"0x1p-1022");
EXPECT_EQ(fmt::format("{:#a}", d),
"0x1.p-1022");
...
EXPECT_EQ(fmt::format("{}", fmt::underlying(color::red)),
"0");
EXPECT_EQ(fmt::format("{}", test_ns::color::red),
"0");
}

TEST(format_test, format_string
) {
EXPECT_EQ(fmt::format("{0}", std::string("test")),
"test");
EXPECT_EQ(fmt::format("{0}", std::string("test")),
"test");
EXPECT_EQ(fmt::format("{:?}", std::string("test")),
"\"test\"");
EXPECT_EQ(fmt::format("{:*^10?}", std::string("test")),
"**\"test\"**");
EXPECT_EQ(fmt::format("{:?}", std::string("\test")),
"\"\\test\"");
EXPECT_THROW((void)
fmt::format(fmt::runtime("{:x}"), std::string("test")
),
fmt::format_error);
}
...
EXPECT_EQ("42", fmt::format("{0}",
Answer()
));
EXPECT_EQ("0042", fmt::format("{:04}",
Answer()
));
}
...
EXPECT_EQ(fmt::format("{}", fmt::group_digits(10000000)),
"10,000,000");
EXPECT_EQ(fmt::format("{:8}", fmt::group_digits(1000)),
"   1,000");
}
...
join(v1, v1
+ 3, ", ")));
EXPECT_EQ("(1)", fmt::format("({})",
join(v1, v1
+ 1, ", ")));
EXPECT_EQ("()", fmt::format("({})",
join(v1, v1,
", ")));
EXPECT_EQ("(001, 002, 003)", fmt::format("({:03})",
join(v1, v1
+ 3, ", ")));
...
EXPECT_EQ("1, 2, 3", fmt::format("{0:{1}}",
join(v1, v1
+ 3, ", "), 1));
...
join(v1,
", ")));
EXPECT_EQ("(+01.20, +03.40)", fmt::format("({:+06.2f})",
join(v2,
", ")));

auto v4 = std::vector<test_enum>{ foo, bar, foo };
EXPECT_EQ("0 1 0", fmt::format("{}",
join(v4,
" ")));
}
...
EXPECT_EQ(fmt::format("{}", fmt::join(v, ", ")),
"1, 2, 3");
}
#endif
...
EXPECT_EQ("42", fmt::format("{answer}", "answer"_a =
Answer()
));
}
...
EXPECT_EQ(fmt::to_string(42),
"42");
EXPECT_EQ(fmt::to_string(reinterpret_cast<void*>(0x1234)),
"0x1234");
EXPECT_EQ(fmt::to_string(adl_test::fmt::detail::foo()),
"foo");
EXPECT_EQ(fmt::to_string(foo),
"0");

#if FMT_USE_FLOAT128
EXPECT_EQ(fmt::to_string(__float128(0.5)),
"0.5");
#endif
...
std::vector<char> v;
fmt::format_to(std::back_inserter(v),
"{}", "foo");
EXPECT_EQ(string_view(v.data(), v.size()),
"foo");
}
...
FMT_BEGIN_NAMESPACE
		template<>
		struct formatter<check_back_appender>
		{
...
EXPECT_EQ(fmt::format("{}", check_back_appender{}),
"y");
}
...
namespace test
{
	...
	auto format_as(scoped_enum_as_int) -> int
	{
		return 42;
	}
	...
	auto format_as(scoped_enum_as_string_view) -> fmt::string_view
	{
		return "foo";
	}
	...
	auto format_as(scoped_enum_as_string) -> std::string
	{
		return "foo";
	}
	...
	auto format_as(struct_as_int) -> int
	{
		return 42;
	}
}  // namespace test
...
EXPECT_EQ(fmt::format("{}", test::scoped_enum_as_int()),
"42");
EXPECT_EQ(fmt::format("{}", test::scoped_enum_as_string_view()),
"foo");
EXPECT_EQ(fmt::format("{}", test::scoped_enum_as_string()),
"foo");
EXPECT_EQ(fmt::format("{}", test::struct_as_int()),
"42");
}
...
EXPECT_EQ(fmt::format(loc, "{:L}", 1234567890),
"1,234,567,89,0");
}
