...
EXPECT_STREQ("abc", fmt::string_view("abc").
data()
);
EXPECT_EQ(3u, fmt::string_view("abc").
size()
);

EXPECT_STREQ("defg",
fmt::string_view(std::string("defg")
).
data()
);
EXPECT_EQ(4u,
...
EXPECT_TRUE(string_view("foo")
.starts_with('f'));
EXPECT_FALSE(string_view("foo")
.starts_with('o'));
EXPECT_FALSE (string_view()
.starts_with('o'));
...
template<typename T>
struct mock_buffer final : buffer<T>
{
...
EXPECT_CALL(buffer, do_grow(15)
).
WillOnce(Return(10)
);
EXPECT_CALL(buffer, do_grow(15)
).
WillOnce(Invoke([&buffer](size_t)
{
...
struct mock_visitor
{
	...
	auto operator()(T value) -> test_result
	{
		return visit(value);
	}

	template<typename U>
	auto operator()(U) -> test_result
	{
		unexpected();
		return test_result();
	}
...
template<typename T>
class numeric_arg_test : public testing::Test
{
};
...
enum class arg_id_result
{
	none, empty, index, name
};
...
template<size_t N>
constexpr auto parse_test_specs(const char (& s)[N])
{
...
struct test_format_string_handler
{
	...
	template<typename T>
	constexpr auto on_arg_id(T) -> int
	{
		return 0;
	}
...
template<size_t N>
constexpr bool parse_string(const char (& s)[N])
{
...
FMT_BEGIN_NAMESPACE
		...
		template<>
		struct formatter<enabled_ptr_formatter*>
		{
			auto parse(format_parse_context& ctx) -> decltype(ctx.begin())
			{
				return ctx.begin();
...
FMT_BEGIN_NAMESPACE
		template<>
		struct formatter<const_formattable>
		{
			auto parse(format_parse_context& ctx) -> decltype(ctx.begin())
			{
				return ctx.begin();
		...
		template<>
		struct formatter<nonconst_formattable>
		{
			auto parse(format_parse_context& ctx) -> decltype(ctx.begin())
			{
				return ctx.begin();
...
FMT_BEGIN_NAMESPACE
		template<>
		struct formatter<convertible_to_pointer_formattable>
		{
			auto parse(format_parse_context& ctx) -> decltype(ctx.begin())
			{
				return ctx.begin();
...
EXPECT_EQ(fmt::format("{}", std::byte(42)),
"42");
}
#endif
...
FMT_BEGIN_NAMESPACE
		...
		template<>
		struct formatter<convertible_to_cstring>
		{
...
"foo");
EXPECT_EQ(fmt::format("{}", convertible_to_cstring()),
"bar");
}
...
EXPECT_EQ(fmt::format("{}", test_struct()),
"test");
}
...
{
	explicit operator fmt::string_view() const
	{
		return "foo";
	}
};
...
{
	template<typename... T>
	void make_format_args(const T& ...) = delete;
...
EXPECT_EQ(fmt::format("{}", nonconst_formattable()),
"test");
}
