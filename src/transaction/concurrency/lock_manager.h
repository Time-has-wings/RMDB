...
class LockManager {
    ...
    enum class LockMode { SHARED, EXLUCSIVE, INTENTION_SHARED, INTENTION_EXCLUSIVE, S_IX, NO_LOCK };
    ...
    public:
        LockRequest(txn_id_t txn_id, LockMode lock_mode)
            : txn_id_(txn_id), lock_mode_(lock_mode), granted_(false), waiting_lock_mode_(LockMode::NO_LOCK) {}

        txn_id_t txn_id_;   // 申请加锁的事务ID
        LockMode lock_mode_;    // 事务申请加锁的类型
        bool granted_;          // 该事务是否已经被赋予锁
        LockMode waiting_lock_mode_;  // granted_为true时,表示该事务已经获得了该数据项的lock_mode_锁,但是现在在申请waiting_lock_mode_
    };
    ...
    public:
        ...
        txn_id_t oldest_txn_id = 2e9; // 优先级最高的事务id(初始化为一个较大值)
