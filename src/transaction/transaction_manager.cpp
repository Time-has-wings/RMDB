...
void TransactionManager::abort(Transaction* txn, LogManager* log_manager)
{
	...
	while (write_set->size())
	{
		...
		{
			sm_manager_->rollback_delete(write_set->back()->GetTableName(),
				write_set->back()->GetRid(),
				write_set->back()->GetRecord(),
				context);
		}
		else if (cur_type == WType::UPDATE_TUPLE)
		{
			sm_manager_->rollback_update(write_set->back()->GetTableName(),
				write_set->back()->GetRid(),
				write_set->back()->GetRecord(),
				context);
		}
