	...
	if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse))
	{
		...
		for (auto& sv_sel_col : x->cols)
		{
			...
			if (auto group = std::dynamic_pointer_cast<ast::GroupValue>(sv_sel_col))
			{
				if (!group->all)
				{
					sel_col = { .tab_name = sv_sel_col->tab_name, .col_name = sv_sel_col->col_name, .as_name = group
						->as_name, .func_name = group->func_name, .isGroup = true };
				}
				else
				{
					group_all = true;
		...
		if (query->cols.empty())
		{
			...
			for (auto& col : all_cols)
			{
				...
				if (group_all)
				{
					sel_col = { .tab_name = col.tab_name, .col_name = col
						.name, .as_name = as_name, .func_name = func_name, .isGroup = true };
					query->cols.push_back(sel_col);
					break;
				}
				else
