...
std::shared_ptr<Query> Analyze::do_analyze(std::shared_ptr<ast::TreeNode> parse)
{
	...
	if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse))
	{
		...
		if (x->group_func)
		{
			...
			TabCol sel_col;
			sel_col = { .tab_name = group->tab_name, .col_name = group->col_name, .as_name = group
				->as_name, .isGroup = true };
		...
		{
			for (const auto& order : x->order_clause->orders)
			{
	...
	else if (auto x = std::dynamic_pointer_cast<ast::UpdateStmt>(parse))
	{
		...
		for (auto& sv_val : x->set_clauses)
		{
			...
			if (auto src_clause = std::dynamic_pointer_cast<ast::SetClause>(sv_val))
			{
				...
				clause.is_update_col = src_clause->is_update_col;
				if (src_clause->is_update_col)
					clause.op = src_clause->op;
